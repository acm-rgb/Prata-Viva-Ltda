...
# ======================================================================================================================
st.set_page_config(
    layout="wide", page_icon="📈", page_title="Vistto - Inteligência de Negócios"
)
...
def extrair_valor_monetario_limpo(valor_texto):
    ...
    if pd.isna(valor_texto) or valor_texto == "":
        return 0.0
...
def padronizar_coluna_texto(serie_pandas):
    ...
    return (
        serie_pandas.astype(str)
        .str.lower()
        .str.strip()
        .apply(remover_acentuacao_e_caracteres_especiais)
    )
...
def executar_pipeline_etl_vistto():
    ...
    df_vendas_bruto = ler_csv_com_fallback_de_codificacao(
        CAMINHO_ARQUIVO_VENDAS, tipos_vendas
    )
    df_clientes_bruto = ler_csv_com_fallback_de_codificacao(
        CAMINHO_ARQUIVO_CLIENTES, tipos_clientes
    )
    df_despesas_bruto = ler_csv_com_fallback_de_codificacao(
        CAMINHO_ARQUIVO_DESPESAS, tipos_despesas
    )
    df_produtos_bruto = ler_csv_com_fallback_de_codificacao(
        CAMINHO_ARQUIVO_PRODUTOS, tipos_produtos
    )

    # 3. Limpeza e Tratamento da Base de Vendas
    df_vendas_limpo = df_vendas_bruto.copy()
    df_vendas_limpo["data_formatada"] = pd.to_datetime(
        df_vendas_limpo["data"], dayfirst=True, errors="coerce"
    )
    df_vendas_limpo["valor_total_numerico"] = df_vendas_limpo["valor_total"].apply(
        extrair_valor_monetario_limpo
    )
    df_vendas_limpo["quantidade_inteira"] = (
        pd.to_numeric(df_vendas_limpo["quantidade"], errors="coerce")
        .fillna(0)
        .astype(int)
    )

    # Removemos vendas zeradas ou canceladas
    df_vendas_limpo = df_vendas_limpo[
        df_vendas_limpo["valor_total_numerico"] > 0
    ].copy()

    # Padroniza nome do produto consultando o dicionário Vistto
    df_vendas_limpo["produto_padronizado"] = padronizar_coluna_texto(
        df_vendas_limpo["produto"]
    ).map(lambda nome: MAPA_PADRONIZACAO_PRODUTOS.get(nome, nome))

    # 4. Limpeza e Tratamento da Base de Produtos
    df_produtos_limpo = df_produtos_bruto.copy()
    df_produtos_limpo["custo_numerico"] = df_produtos_limpo["custo_unitario"].apply(
        extrair_valor_monetario_limpo
    )
    df_produtos_limpo["estoque_atual_numerico"] = (
        pd.to_numeric(df_produtos_limpo["estoque_atual"], errors="coerce")
        .fillna(0)
        .abs()
    )
    df_produtos_limpo["estoque_minimo_numerico"] = (
        pd.to_numeric(df_produtos_limpo["estoque_minimo"], errors="coerce")
        .fillna(0)
        .abs()
    )
    df_produtos_limpo["produto_padronizado"] = padronizar_coluna_texto(
        df_produtos_limpo["descricao_padrao"]
    )

    # 5. Limpeza e Deduplicação da Base de Clientes
    df_clientes_limpo = df_clientes_bruto.copy()
    df_clientes_limpo["telefone_apenas_numeros"] = (
        df_clientes_limpo["telefone"].astype(str).str.replace(r"\D", "", regex=True)
    )
    df_clientes_limpo = df_clientes_limpo.drop_duplicates(
        subset=["id_cliente"]
    ).reset_index(drop=True)

    # 6. Limpeza e Tratamento da Base de Despesas
    df_despesas_limpo = df_despesas_bruto.copy()
    df_despesas_limpo["data_formatada"] = pd.to_datetime(
        df_despesas_limpo["data"], dayfirst=True, errors="coerce"
    )
    df_despesas_limpo["valor_numerico"] = df_despesas_limpo["valor"].apply(
        extrair_valor_monetario_limpo
    )
    df_despesas_limpo["categoria_padronizada"] = padronizar_coluna_texto(
        df_despesas_limpo["categoria"]
    ).map(lambda cat: MAPA_PADRONIZACAO_DESPESAS.get(cat, cat))
    ...
    df_vendas_limpo = df_vendas_limpo.merge(
        df_produtos_limpo[
            ["produto_padronizado", "custo_numerico", "categoria", "codigo"]
        ],
    ...
    df_vendas_limpo["custo_total_da_venda"] = (
        df_vendas_limpo["quantidade_inteira"] * df_vendas_limpo["custo_numerico"]
    )
    df_vendas_limpo["lucro_bruto_da_venda"] = (
        df_vendas_limpo["valor_total_numerico"]
        - df_vendas_limpo["custo_total_da_venda"]
    )
...
df_vendas_base, df_clientes_base, df_despesas_base, df_produtos_base = (
    executar_pipeline_etl_vistto()
)
...
with st.sidebar:
    ...
    df_vendas_base["competencia_mes_ano"] = df_vendas_base[
        "data_formatada"
    ].dt.strftime("%m/%Y")

    lista_de_competencias_disponiveis = sorted(
        df_vendas_base["competencia_mes_ano"].dropna().unique().tolist(), reverse=True
    )

    periodo_selecionado = st.selectbox(
        "Período de Análise", ["Todo o Histórico"] + lista_de_competencias_disponiveis
    )
...
if periodo_selecionado != "Todo o Histórico":
    df_vendas_filtrado = df_vendas_base[
        df_vendas_base["competencia_mes_ano"] == periodo_selecionado
    ].copy()

    # Precisamos calcular o mês/ano das despesas em tempo real para o filtro
    mascara_mes_despesa = (
        df_despesas_base["data_formatada"].dt.strftime("%m/%Y") == periodo_selecionado
    )
...
aba_executiva, aba_performance, aba_estoque, aba_financeiro = st.tabs(
    ["📊 Executivo", "🎯 Performance", "📦 Estoque", "💸 Financeiro"]
)
...
with aba_executiva:
    ...
    with st.container(border=True):
        ...
        ticket_medio_periodo = (
            faturamento_total_periodo / quantidade_total_vendas
            if quantidade_total_vendas > 0
            else 0
        )
...
with aba_performance:
    ...
    with st.container(border=True):
        ...
        investimento_marketing_periodo = df_despesas_filtrado[
            df_despesas_filtrado["categoria_padronizada"] == "marketing"
        ]["valor_numerico"].sum()
        total_clientes_base = df_clientes_base.shape[0]
        custo_aquisicao_cliente = (
            investimento_marketing_periodo / total_clientes_base
            if total_clientes_base > 0
            else 0
        )

        clientes_unicos_com_compra = df_vendas_filtrado["id_cliente"].nunique()
        lifetime_value_estimado = (
            faturamento_total_periodo / clientes_unicos_com_compra
            if clientes_unicos_com_compra > 0
            else 0
        )

        coluna_metrica_1.metric(
            "Investimento Marketing", f"R$ {investimento_marketing_periodo:,.2f}"
        )
        coluna_metrica_2.metric(
            "CAC Geral Estimado", f"R$ {custo_aquisicao_cliente:,.2f}"
        )
...
with aba_estoque:
    ...
    df_analise_estoque["valor_total_imobilizado"] = (
        df_analise_estoque["estoque_atual_numerico"]
        * df_analise_estoque["custo_numerico"]
    )

    montante_total_imobilizado = df_analise_estoque["valor_total_imobilizado"].sum()
    df_produtos_em_ruptura = df_analise_estoque[
        df_analise_estoque["estoque_atual_numerico"]
        <= df_analise_estoque["estoque_minimo_numerico"]
    ]
    quantidade_itens_ruptura = len(df_produtos_em_ruptura)

    coluna_estoque_1, coluna_estoque_2 = st.columns(2)
    coluna_estoque_1.metric(
        "Capital Imobilizado", f"R$ {montante_total_imobilizado:,.2f}"
    )
    ...
    if not df_produtos_em_ruptura.empty:
        st.error(
            "🚨 Atenção: Identificamos produtos abaixo da margem de segurança do estoque mínimo!"
        )
        st.dataframe(
            df_produtos_em_ruptura[
                [
                    "codigo",
                    "descricao_padrao",
                    "estoque_atual_numerico",
                    "estoque_minimo_numerico",
                ]
            ],
...
with aba_financeiro:
    ...
    with st.container(border=True):
        ...
        coluna_fin_1.metric(
            "Total de Entradas (Vendas)", f"R$ {faturamento_total_periodo:,.2f}"
        )
        coluna_fin_2.metric(
            "Total de Saídas (Despesas)", f"R$ {despesas_totais_periodo:,.2f}"
        )
        coluna_fin_3.metric(
    ...
    with st.container(border=True):
        ...
        df_entradas_diarias = (
            df_vendas_filtrado.groupby(df_vendas_filtrado["data_formatada"].dt.date)[
        ...
        df_saidas_diarias = (
            df_despesas_filtrado.groupby(
                df_despesas_filtrado["data_formatada"].dt.date
            )["valor_numerico"]
            .sum()
            .reset_index(name="valor_transacionado")
        )
